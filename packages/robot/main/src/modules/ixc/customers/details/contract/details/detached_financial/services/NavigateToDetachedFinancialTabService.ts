...
@injectable()
export default class NavigateToDetachedFinancialTabService {
  constructor(
    @inject('Page')
    private page: Page,
  ) {}
  ...
  public async execute(): Promise<void> {
    ...
    const [
      findDetachedFinancialTabElement,
    ] = await this.page.findElementsBySelector(
      'form[id="3_form"] div.abas.clearfix > ul > li:nth-child(12) > a',
    );

    await findDetachedFinancialTabElement.click();

    await this.page.driver.waitForSelector(
      'div.panel.mostrando[id="11"] > dl > div > div > div.hDiv > div > table',
    );
  }
}
