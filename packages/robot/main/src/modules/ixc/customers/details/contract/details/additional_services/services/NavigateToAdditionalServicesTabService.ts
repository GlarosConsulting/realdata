...
@injectable()
export default class NavigateToAdditionalServicesTabService {
  constructor(
    @inject('Page')
    private page: Page,
  ) {}
  ...
  public async execute(): Promise<void> {
    ...
    const [
      ...
      'form[id="3_form"] div.abas.clearfix > ul > li:nth-child(8) > a',
    );

    await findAdditionalServicesTabElement.click();

    await this.page.driver.waitForSelector(
      '#cliente_contrato_cliente_contrato_servicos',
    );
  }
}
