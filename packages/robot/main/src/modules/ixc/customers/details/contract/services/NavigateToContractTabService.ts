...
@injectable()
export default class NavigateToContractTabService {
  constructor(
    @inject('Page')
    private page: Page,
  ) {}
  ...
  public async execute(): Promise<void> {
    ...
    const [
      ...
      'form > div.abas.clearfix > ul > li:nth-child(7) > a',
    );

    await findContractTabButtonElement.click();

    await this.page.driver.waitForSelector('#cliente_cliente_contrato');
  }
}
